...
<div class="container-fluid px-3 px-md-4 py-3 py-md-4">
    <div class="d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between mb-3 mb-md-4 gap-3">
        <div>
            <h1 class="h4 fw-bold mb-1">Order #{{ $order->id }}</h1>
            <p class="text-muted small mb-0">Placed {{ $order->created_at->format('M d, Y \a\t H:i') }}</p>
        </div>
        <div class="d-flex gap-2 w-100 w-md-auto">
            <a href="{{ route('admin.dashboard') }}" class="btn btn-outline-secondary w-100 w-md-auto">Back to Dashboard</a>
            <a href="{{ route('admin.dashboard', request()->only('status')) }}" class="btn btn-dark w-100 w-md-auto">Orders</a>
        </div>
    </div>
    ...
    <div class="row g-3 g-md-4">
                ...
                <div class="card-body">
                    <div class="d-flex flex-column gap-1">
                        <div>
                            ...
                            <span class="fw-semibold">{{ $order->shipping_info['full_name'] ?? 'N/A' }}</span>
                        </div>
                        <div>
                            <span class="text-muted small">Address:</span>
                            <span class="fw-semibold">{{ $order->shipping_info['address'] ?? 'N/A' }}</span>
                        </div>
                        <div>
                            <span class="text-muted small">City/State:</span>
                            <span class="fw-semibold">{{ ($order->shipping_info['city'] ?? 'N/A') }}{{ isset($order->shipping_info['state']) ? ', ' . $order->shipping_info['state'] : '' }}</span>
                        </div>
                        <div>
                            <span class="text-muted small">Zip:</span>
                            <span class="fw-semibold">{{ $order->shipping_info['zip_code'] ?? 'N/A' }}</span>
                        </div>
                        <div>
                            <span class="text-muted small">Phone:</span>
                            <span class="fw-semibold">{{ $order->shipping_info['phone'] ?? 'N/A' }}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    ...
                    @if(!empty($order->stripe_payment_intent_id))
                        <div class="d-flex align-items-center justify-content-between mt-2">
                            ...
                            <span class="fw-semibold">{{ $order->stripe_payment_intent_id }}</span>
                        </div>
                    ...
                    @if(!empty($order->paid_at))
                        <div class="d-flex align-items-center justify-content-between mt-2">
                            ...
                            <span class="fw-semibold">{{ $order->paid_at->format('M d, Y H:i') }}</span>
                        </div>
                    @endif
                    @if(!empty($order->cancelled_at))
                        <div class="d-flex align-items-center justify-content-between mt-2">
                            <span class="text-muted small">Cancelled at</span>
                            <span class="fw-semibold">{{ $order->cancelled_at->format('M d, Y H:i') }}</span>
                        </div>
                    @endif
                    @if(!empty($order->restocked_at))
                        <div class="d-flex align-items-center justify-content-between mt-2">
                            <span class="text-muted small">Restocked at</span>
                            <span class="fw-semibold">{{ $order->restocked_at->format('M d, Y H:i') }}</span>
                        </div>
                    @endif

                    @if(empty($order->cancelled_at) && ($order->status !== 'delivered'))
                        <form action="{{ route('admin.orders.cancel', $order) }}" method="POST" class="mt-3">
                            @csrf
        ...
        <div class="col-12 col-lg-8">
                            ...
                            <tbody>
                                @foreach($order->items as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="text-muted small">Size {{ $item->size }}, {{ $item->color }}</span>
                                        </td>
            ...
            <div class="card mt-3">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.orders.update', $order) }}" method="POST" class="d-flex flex-column flex-md-row align-items-start align-items-md-end gap-2">
                        @csrf
                        @method('PATCH')
                        <div class="w-100">
